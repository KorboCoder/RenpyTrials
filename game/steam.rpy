init python:
    ...
    def Steam(image, max_particles=50, speed=150, wind=100, rotate=0.0,  xborder=(-50,100), yborder=(-50,400), **kwargs):

        return Particles(SteamFactory(image, max_particles, speed, rotate, wind, xborder, yborder, **kwargs))

    class SteamFactory(object):
        ...
        def create(self, particles, st):
            ...
            for _i in xrange(0, self.max_particles):
                ...
                rv.append( SteamParticle(self.image[depth-1],      # the image used by the particle
    ...
    class SteamParticle(object):
        def __init__(self, image, wind, speed, direction, xborder, yborder):

            """
            Initializes the Steam particle. This is called automatically when the object is created.
            """
            
            # The image used by this particle
            self.image = image
            
            # For safety (and since we don't have Steam going from the floor to the sky o.o)
        ...
        def update(self, st):
            ...
            if self.ypos > renpy.config.screen_height or\
               (self.wind< 0 and self.xpos < 0) or (self.wind > 0 and self.xpos > renpy.config.screen_width):
                return None
            #self.image = im.FactorScale(self.image, self.width*0.5, self.height*0.5)
